...
from src.utils import (
    EarlyStopping, CheckpointManager, get_logger, count_parameters
)
...
class Trainer:
    def __init__(self, model, cfg, output_dir: str, wandb_run=None):
        ...
        # fp16 scaler
        self.use_fp16 = bool(getattr(cfg.training, "fp16", False)) and self.device.type == "cuda"
    ...
    def _build_optimizer_scheduler(self, num_training_steps: int):
        ...
        bert_params  = list(self.model.bert.parameters()) if hasattr(self.model, "bert") else []
        other_params = [
        ...
        optimizer = torch.optim.AdamW(
            [
                {"params": bert_params,  "lr": tcfg.bert_lr} if bert_params else {"params": [], "lr": tcfg.bert_lr},
    ...
    def _train_epoch(self, loader, optimizer, scheduler) -> float:
        ...
        for step, batch in enumerate(tqdm(loader, desc="  train", leave=False, dynamic_ncols=True)):
            ...
            # scheduler step phải đặt SAU optimizer.step / scaler.step
            scheduler.step()
    ...
    def train(self, train_loader, dev_loader, evaluator) -> dict:
        ...
        for epoch in range(1, tcfg.epochs + 1):
            ...
            logger.info(f"  loss={train_loss:.4f} | dev_f1={dev_f1:.4f} | time={elapsed:.1f}s")
            print(f"[Epoch {epoch}/{tcfg.epochs}] loss={train_loss:.4f} | dev_f1={dev_f1:.4f} | time={elapsed:.1f}s",
                  flush=True)
            ...
            # Free GPU cache sau mỗi epoch (an toàn, nhưng không bắt buộc)
